...
function getFiles()
{
    ...
    if (!$rs) {
        ...
        foreach ($rs as $a) {
            ...
            $edit = span(href("<img src='Recursos/img/Archivos-edit.gif' />",
                "?a=$go[a]&amp;q=editfile&amp;id=$a[media_id]"), "class='p-action'");

            $url = BASEURL . '/Archivos/' . $a['media_file'];
...
function linksManager()
{
    ...
    foreach ($newarr as $key => $out) {
        ...
        foreach ($out as $go) {
            $p .= "<option value=\"<a href='" . BASEURL . gstl_rewriter($go[0]) . "' alt='' title='" . htmlspecialchars($go[1]) . "'>$go[1]\">$go[1]</option>\n";

            // hackery
            //$p .= "<option value=\"<a href='".BASEURL.gstl_rewriter($go[0])."' alt='' title='".htmlspecialchars($go[1])."'>$go[1]</a>\">$go[1]</option>\n";
        }
...
function filesManagerType($type, $file, $x = '', $y = '', $desc = '')
{
    ...
    if (in_array($type, $uploads['images'])) {
        return href("<img src='Recursos/img/Archivos-use.gif' />", '#', "onClick=\"ModInsImg('" . BASEURL . '/Archivos/' . $file . "','$x','$y');return false;\"");
    } // mp3, mov, etc...
    elseif (in_array($type, $uploads['media'])) {
        switch ($type) {
            case 'mp3':
                return href("<img src='Recursos/img/Archivos-use.gif' />", '#', "onClick=\"ModInsMP3('" . BASEURL . '/Archivos/' . $file . "');return false;\"");
                break;
            case 'mov':
                return href("<img src='Recursos/img/Archivos-use.gif' />", '#', "onClick=\"ModInsMov('" . BASEURL . '/Archivos/' . $file . "','$x','$y');return false;\"");
                break;
            // not in use...
            case 'avi':
                return href("<img src='Recursos/img/Archivos-use.gif' />", '#', "onClick=\"ModInsAVI('" . BASEURL . '/Archivos/' . $file . "','$x','$y');return false;\"");
                break;
            case 'jar':
                return href("<img src='Recursos/img/Archivos-use.gif' />", '#', "onClick=\"ModInsJAR('" . BASEURL . '/Archivos/' . $file . "','$x','$y');return false;\"");
                break;
        }
    } // flash
    elseif (in_array($type, $uploads['flash'])) {
        return href("<img src='Recursos/img/Archivos-use.gif' />", '#', "onClick=\"ModInsFlash('" . BASEURL . '/Archivos/' . $file . "','$x','$y');return false;\"");
    } // other Archivos
    else {
        return href("<img src='Recursos/img/Archivos-use.gif' />", '#', "onClick=\"ModInsFile('" . BASEURL . '/Archivos/' . $file . "','$desc');return false;\"");
    }
}
