...
class System extends Router
{
    ...
    function page_prefs()
    {
        ...
        load_helper('output');
        load_module_helper('Archivos', $go['a']);
    ...
    function page_files()
    {
        ...
        $this->template->pop_location = $this->lang->word('Archivos manager');

        $this->template->pop_links[] = array($this->lang->word('upload Archivos'), "?a=$go[a]&amp;q=upload");

        // ++++++++++++++++++++++++++++++++++++++

        $body = div(getFiles(), "id='p-Archivos'");
    ...
    function page_editfile()
    {
        ...
        $this->template->pop_links[] = array($this->lang->word('Archivos manager'), "?a=$go[a]&amp;q=Archivos");
    ...
    function page_upload()
    {
        ...
        $allowed = array_merge($uploads['images'], $uploads['media'], $uploads['Archivos'], $uploads['flash']);
    ...
    {
        global $go;
        $processor =& load_class('processor', TRUE, 'Libreria');
    ...
    function sbmt_upd_file()
    {
        ...
        if (isset($_POST['upd_delfile'])) {
            ...
            if ($file) {
                ...
                if (file_exists(DIRNAME . '/Archivos/' . $file['media_file'])) {
                    //$this->BaseDatos->deleteArray(PX.'media', "media_id='$go[id]'");
                    unlink(DIRNAME . '/Archivos/' . $file['media_file']);
                }
            }
        } else {
            $processor =& load_class('processor', TRUE, 'Libreria');
        ...
        system_redirect("?a=$go[a]&q=Archivos");
    }
    ...
    function sbmt_upd_files()
    {
        ...
        $IMG =& load_class('media', TRUE, 'Libreria');
        ...
        $types = array_merge($uploads['images'], $uploads['media'], $uploads['Archivos'], $uploads['Archivos'], $uploads['flash']);
        $IMG->path = $dir;
        ...
        if ($num > 0) {
            for ($i = 0; $i < $num; $i++) {
                if ($_FILES['filename']['size'][$i] < $IMG->upload_max_size) {
                    ...
                    load_helper('output');
                    $URL =& load_class('publish', TRUE, 'Libreria');
