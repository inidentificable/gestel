...
class Exhibits extends Router
{
    ...
    function page_settings()
    {
        ...
        if ($rs['obj_mode'] == 1) {
            ...
            $body .= getThemes(DIRNAME . BASENAME . '/Sitio/', $rs['obj_theme']);

            $body .= ips($this->lang->word('organize'), 'getOrganize', 'obj_org', $rs['obj_org']);
        }

        //$body .= ips($this->Lenguaje->word('use editor'), 'getGeneric', 'writing', $rs['writing']);
    ...
    function page_edit()
    {
        ...
        $this->template->add_js('gstl.exhibit-edit.js');
        ...
        $body .= getPresent(DIRNAME . BASENAME . '/Sitio/plugin/', $rs['format']);
...
//		$body .= ips($this->Lenguaje->word('image title'), 'input', 'media_title',
//			$rs['media_title'], "id='media_title' maxlength='35'", 'text');
//		$body .= ips($this->Lenguaje->word('image caption'), 'input', 'media_caption',
//		$rs['media_caption'], "id='media_caption' maxlength='35'", 'text');
    ...
    function page_jxbg()
    {
        ...
        if (isset($_POST['upload'])) {
            if (isset($_POST['deletion'])) {
                ...
                if (file_exists(DIRNAME . '/Archivos/' . $filename)) {
                    unlink(DIRNAME . '/Archivos/' . $filename);
                }
            } else {
                // perform the upload
    ...
    function sbmt_add_sec()
    {
        ...
        if ($processor->check_errors()) {
            ...
            $folder_name = load_class('publish', TRUE, 'Libreria');
    ...
    function sbmt_publish_x()
    {
        ...
        if ($default['reporting'] == TRUE) {
            if (($rs['report'] != 1) && ($rs['obj_apikey'] != '')
                ...
                $REST =& load_class('rest', TRUE, 'Libreria');
...
<a href=\'<%baseurl%><plug:gstl_rewriter url=\'/about-this-Sitio/\' />\'>" . $this->lang->word('about this Sitio') . "</a></p>";
            } else {
                if ($clean['obj_itop'] == '') {
                    $clean['obj_itop'] = "<p><%obj_name%><br />
<a href=\'<%baseurl%><plug:gstl_rewriter url=\'/about-this-Sitio/\' />\'>" . $this->lang->word('about this Sitio') . "</a></p>";
