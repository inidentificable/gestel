...
class Front
{
    ...
    function init_front()
    {
        if (file_exists(DIRNAME . BASENAME . '/Sitio/plugin/exhibit.' . $this->result['format'] . '.php')) {
            include DIRNAME . BASENAME . '/Sitio/plugin/exhibit.' . $this->result['format'] . '.php';
        } else {
            // thie default format
            include DIRNAME . BASENAME . '/Sitio/plugin/exhibit.grow.php';
        }
        ...
        if ($this->exhibit['lib_css'] != '') {
            if (is_array($this->exhibit['lib_css'])) {
                foreach ($this->exhibit['lib_css'] as $css) {
                    $out .= "<style type='text/css'> @import url(" . BASEURL . BASENAME . "/Sitio/css/$css); </style>\n";
                }
            } else {
                $out .= "<style type='text/css'> @import url(" . BASEURL . BASENAME . "/Sitio/css/" . $this->exhibit['lib_css'] . "); </style>\n";
            }
        }
        ...
        if ($this->exhibit['lib_js'] != '') {
            if (is_array($this->exhibit['lib_js'])) {
                foreach ($this->exhibit['lib_js'] as $js) {
                    $out .= "<script type='text/javascript' src='" . BASEURL . BASENAME . "/Sitio/js/$js'></script>\n";
                }
            } else {
                $out .= "<script type='text/javascript' href='" . BASEURL . BASENAME . "/Sitio/js/" . $this->exhibit['lib_js'] . "'></script>\n";
            }
        }
    ...
    function front_eatock()
    {
		...
		<html xmlns='http://www.w3.org/1999/xhtml' xml:Lenguaje='en' Lenguaje='en'>
		<head>
		<meta http-equiv='Content-Type' content='text/html; charset=utf-8'/>

		<title><%title%> : <%obj_name%></title>

		<link rel='stylesheet' href='<%baseurl%><%basename%>/Sitio/<%obj_theme%>/style.css' type='text/css' />
		<!--[if lte IE 6]>
		<link rel='stylesheet' href='<%baseurl%><%basename%>/Sitio/<%obj_theme%>/ie.css' type='text/css' />
		<![endif]-->
		<plug:front_lib_css />
		<plug:front_dyn_css />
		<script type='text/javascript' src='<%baseurl%><%basename%>/Sitio/js/jquery.js'></script>
		<script type='text/javascript' src='<%baseurl%><%basename%>/Sitio/js/cookie.js'></script>
		<plug:front_lib_js />
		...
		<!-- you must provide a link to Indexhibit on your Sitio someplace - thank you -->
		<ul>
