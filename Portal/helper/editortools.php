...
function editorTools($content='', $advanced=FALSE, $additional='', $process='')
{
	...
	if ($OBJ->access->prefs['writing'] != 1) {
		...
		if ($advanced == 1)
		{
			$s .= href("<img src='asset/img/Archivos.gif' alt'[]' />",'#',"title='".$OBJ->lang->word('Archivos manager')."' class='btn btn-off' onmouseover=\"this.className='btn btn-over'\" onmouseout=\"this.className='btn btn-off'\" onClick=\"OpenWindow('?a=system&amp;q=Archivos','popup','700','465','yes');return false;\"");
		}
